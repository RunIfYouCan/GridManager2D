...
func _draw() -> void:
	...
	var sorted_layers: Array[GridLayer] = layers.duplicate()
...
func cell_center(cell: Vector2i, cfg: GridConfig) -> Vector2:
	...
	var y: float = cfg.grid_origin.y + cell.y * cfg.cell_size.y * 0.75 + cfg.cell_size.y * 0.5
	return Vector2(x, y)


func hex_polygon(center: Vector2, cell_size: Vector2) -> PackedVector2Array:
	var points: PackedVector2Array = PackedVector2Array()
	var rx: float = cell_size.x * 0.5
	var ry: float = cell_size.y * 0.5
	for i in range(6):
		var angle: float = deg_to_rad(60.0 * i - 30.0)  # pointy-top: first point at top
		points.append(center + Vector2(rx * cos(angle), ry * sin(angle)))
	return points


func _draw_cell(cell: Vector2i, layer: GridLayer) -> void:
	var center: Vector2 = cell_center(cell, grid_config)
	if grid_config.tile_shape == GridManager.TileShape.SQUARE:
		var rect: Rect2 = Rect2(center - grid_config.cell_size * 0.5, grid_config.cell_size)
		draw_rect(rect, layer.fill_color)
		if layer.border_width > 0.0:
			draw_rect(rect, layer.border_color, false, layer.border_width)
	else:
		var polygon: PackedVector2Array = hex_polygon(center, grid_config.cell_size)
